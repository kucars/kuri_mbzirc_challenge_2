...
# define state : explore_wp
class explore_wp(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['updated','preempted'])
     
    def execute(self, userdata):
        rospy.loginfo('Executing state EXPLORE_WP')
        #userdata.explore_wp_out = userdata.explore_wp_in + 1
        time.sleep(1)
        if self.preempt_requested():
                self.service_preempt()
                return 'preempted'
...
class Challenge2():
    def __init__(self):
        ...
        self.searching=smach.StateMachine(outcomes=['terminated'])
        with self.searching:
        # Add states to the container
        	smach.StateMachine.add('SEARCHING_PANEL_IN_MAP', panel_searching(), 
                               	transitions={'not_found':'SEARCHING_PANEL_IN_MAP','found':'terminated'})
       

         
        self.exploring=smach.StateMachine(outcomes=['preempted'])
        with self.exploring:
        # Add states to the container
        	smach.StateMachine.add('EXPLORING', explore_wp(), 
                               	transitions={'updated':'EXPLORING'})   

        with self.sm_con:
         	smach.Concurrence.add('SEARCHING', self.searching)
    ...
    def concurrence_child_termination_cb(self, outcome_map):
    	...
    	if outcome_map['SEARCHING'] == 'terminated':
